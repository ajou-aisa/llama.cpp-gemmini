...
enum class WeightScaleInfoMode {
  CommonOutput,
  Residual,
  // NPU HP1 consumes the original carrier, not an expanded integer factor.
  ResidualHp1Scu,
};
...
struct WeightRoutePlan {
  ...
  bool valid = false;
  bool hp1_carriers = false; // explicit NPU/reference scale-validation domain
};
...
inline bool is_q8_h2_args(const ggml_gemmini_args_t &args) {
  return args.weight_format ==
         ...
         args.q8_h2_blocks != nullptr && args.q8_h2_blocks_per_row > 0;
}

inline bool is_q8_hp1_args(const ggml_gemmini_args_t &args) {
  return args.weight_format ==
             ggml_gemmini_args_t::im2p_weight_format_t::q8_hp1 &&
         args.q8_hp1_blocks != nullptr && args.q8_hp1_block_count > 0 &&
...
inline bool is_q8_hp2_args(const ggml_gemmini_args_t &args) {
  return args.weight_format ==
         ...
         args.q8_hp2_blocks != nullptr && args.q8_hp2_block_count > 0 &&
...
inline bool has_q8_hp1_native_contract(const ggml_gemmini_args_t &args) {
  return args.B == nullptr && !args.weight_i8_scale_active &&
         is_q8_hp1_args(args) && args.has_q8_hp1_im2p_contract();
}

inline bool has_q8_hp2_native_contract(const ggml_gemmini_args_t &args) {
  return args.B == nullptr && !args.weight_i8_scale_active &&
         is_q8_hp2_args(args) && args.has_q8_hp2_im2p_contract();
}

inline bool is_q8_h1_weight_args(const ggml_gemmini_args_t &args) {
  return is_q8_h1_args(args) ||
         (args.B && !args.B_scales && args.c_b &&
...
inline WeightLayout resolve_weight_layout(const ggml_gemmini_args_t &args) {
  if (is_q8_channel_direct_read_args(args) ||
