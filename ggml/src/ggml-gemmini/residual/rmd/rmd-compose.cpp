...
RmdStatus merge_rmd_correction_checked(
    const ggml_gemmini_args_t &args, const MergeLayout &layout,
    const wroute::WeightRoutePlan &plan,
  ...
  const auto *fully_scaled =
      std::get_if<FullyScaledFloat64Correction>(&correction);
  ...
  // Publish the count only after the whole merge succeeds, just like the
  // destination.
  if (nonzero_count != nullptr)
    *nonzero_count = staged_nonzero_count;
  return RmdStatus::success;
}
...
RmdStatus merge_rmd_correction_to(const ggml_gemmini_args_t &args,
  ...
  std::vector<float> column_scale;
  const RmdStatus scales =
      prepare_column_scales(args, plan, nullptr, column_scale);
  if (scales != RmdStatus::success)
    return scales;
  return merge_rmd_correction_checked(args, layout, plan, column_scale,
                                      correction, nonzero_count);
}
...
RmdStatus merge_rmd_correction_with_weights(const ggml_gemmini_args_t &args,
  ...
  if (scales != RmdStatus::success)
    return scales;
  return merge_rmd_correction_checked(args, layout, plan, weights.column_scale_,
                                      correction, nonzero_count);
}
...
RmdStatus merge_rmd_correction(const ggml_gemmini_args_t &args,
                               ...
                               const Correction &correction,
                               size_t *nonzero_count) {
  return merge_rmd_correction_to(args, args.f_out, global_row_begin,
                                 global_row_end, correction, nonzero_count);
}
