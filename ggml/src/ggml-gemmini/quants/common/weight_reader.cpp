...
bool checked_block_offset(size_t row, size_t blocks_per_row, size_t block,
...
bool native_extent_covers(const ggml_gemmini_args_t &args, size_t block_count,
                          size_t block_bytes) {
...
bool dense_extent_is_addressable(const ggml_gemmini_args_t &args,
...
WeightReaderStatus validate_dense_q8_h1(const ggml_gemmini_args_t &args,
                                        const wroute::WeightRoutePlan &plan) {
...
WeightReaderStatus validate_storage(const ggml_gemmini_args_t &args,
                                    const wroute::WeightRoutePlan &plan) {
#if defined(GGML_GEMMINI_TESTING)
  ...
  if (plan.route == Route::Dense || plan.route == Route::Q8ChannelDirect ||
    ...
    return args.B != nullptr && dense_extent_is_addressable(args, plan)
               ? WeightReaderStatus::Success
               : WeightReaderStatus::InvalidMetadata;
  }
  ...
  case Format::q16_hp1:
    if (!args.has_native_matched_width_contract() ||
        !native_extent_covers(
            args, args.native_block_count,
  ...
  case Format::q8_h0:
    ...
    if (args.B_blocks != nullptr) {
      return reinterpret_cast<uintptr_t>(args.B_blocks) % alignof(block_q8_0) ==
                         0 &&
                     native_extent_covers(args, expected_blocks,
                                          sizeof(block_q8_0))
                 ? WeightReaderStatus::Success
                 : WeightReaderStatus::InvalidMetadata;
    }
  ...
  case Format::q8_hp1:
    if (!wroute::has_q8_hp1_native_contract(args) ||
        !native_extent_covers(args, args.q8_hp1_block_count,
                              sizeof(block_q8_hp1))) {
      return WeightReaderStatus::InvalidMetadata;
    }
    return WeightReaderStatus::Success;
...
const Block *native_block(const Block *blocks, size_t block_count,
                          size_t blocks_per_row, size_t row, size_t block) {
  size_t offset = 0;
  return blocks != nullptr && checked_block_offset(row, blocks_per_row, block,
                                                   block_count, offset)
             ? blocks + offset
             : nullptr;
}
...
WeightScaleResult make_hp1_scale(int16_t exponent, float column_scale,
                                 ScaleDomain domain) {
  if (!finite_float(column_scale))
    return invalid_scale(WeightReaderStatus::InvalidMetadata);
  ...
  return make_h1_scale(uint64_t{1} << static_cast<unsigned>(exponent),
                       column_scale);
}

WeightScaleResult read_scale_unchecked(const ggml_gemmini_args_t &args,
                                       const wroute::WeightRoutePlan &plan,
  ...
  switch (args.weight_format) {
  case Format::q4_h0: {
    const block_q4_h0 *block =
        native_block(args.q4_h0_blocks, args.native_block_count,
  ...
  case Format::q8_h0:
    if (args.B_blocks != nullptr) {
      ...
      if (!checked_mul_size(args.blocks_J, args.blocks_K, block_count) ||
          !checked_block_offset(j, args.blocks_K, block_index, block_count,
                                offset)) {
        return invalid_scale(WeightReaderStatus::InvalidMetadata);
      }
      return make_h0_scale(ggml_fp16_to_fp32(args.B_blocks[offset].d));
    }
    return args.B_scales == nullptr
               ? invalid_scale(WeightReaderStatus::InvalidMetadata)
               : make_h0_scale(args.B_scales[j * args.blocks_K + block_index]);
  case Format::q16_h0: {
    const block_q16_h0 *block =
        native_block(args.q16_h0_blocks, args.native_block_count,
                     args.native_blocks_per_row, j, block_index);
    return block == nullptr ? invalid_scale(WeightReaderStatus::InvalidMetadata)
                            : make_h0_scale(ggml_fp16_to_fp32(block->d));
  }
  case Format::q4_h1: {
    const block_q4_h1 *block =
        native_block(args.q4_h1_blocks, args.native_block_count,
                     args.native_blocks_per_row, j, block_index);
    if (block == nullptr ||
        !zero_bytes(block->padding, sizeof(block->padding)) ||
        !zero_bytes(block->tail_padding, sizeof(block->tail_padding))) {
      return invalid_scale(WeightReaderStatus::InvalidMetadata);
    }
    return make_h1_scale(static_cast<uint64_t>(block->c_b) + block->R,
                         block->s_rf);
  }
  case Format::q8_h1: {
    ...
    return block == nullptr
               ? invalid_scale(WeightReaderStatus::InvalidMetadata)
               : make_h1_scale(static_cast<uint64_t>(block->c_b) + block->R,
                               block->s_rf);
  }
  case Format::q8_0_unpacked_to_h1: {
    ...
    const uint64_t offset =
        args.stripe_J > 1 ? args.R_stripe[stripe] : args.R[j];
    const float column =
        args.stripe_J > 1 ? args.s_rf_stripe[stripe] : args.s_rf[j];
    return make_h1_scale(static_cast<uint64_t>(args.c_b[index]) + offset,
                         column);
  }
  case Format::q16_h1: {
    const block_q16_h1 *block =
        native_block(args.q16_h1_blocks, args.native_block_count,
                     args.native_blocks_per_row, j, block_index);
    if (block == nullptr ||
        !zero_bytes(block->padding, sizeof(block->padding)) ||
        !zero_bytes(block->tail_padding, sizeof(block->tail_padding))) {
  ...
  case Format::q4_hp1: {
    const block_q4_hp1 *block =
        native_block(args.q4_hp1_blocks, args.native_block_count,
                     args.native_blocks_per_row, j, block_index);
  ...
  case Format::q16_hp1: {
    const block_q16_hp1 *block =
        native_block(args.q16_hp1_blocks, args.native_block_count,
                     args.native_blocks_per_row, j, block_index);
    if (block == nullptr || !zero_bytes(block->padding, sizeof(block->padding)))
      return invalid_scale(WeightReaderStatus::InvalidMetadata);
    return make_hp1_scale(block->m, block->channel_scale, plan.scale_domain);
...
Hp1CarrierResult read_hp1_carrier_validated(const ggml_gemmini_args_t &args,
                                            const wroute::WeightRoutePlan &plan,
                                            size_t column,
                                            size_t original_block) {
  Hp1CarrierResult result;
  if (plan.route != Route::HP1)
    return result;
  result.status = WeightReaderStatus::InvalidMetadata;
  if (column >= args.J || plan.scales.block_size != 32 ||
      original_block >= plan.scales.cols)
    return result;
  const auto read = [&](const auto *block) {
    if (block == nullptr ||
        !zero_bytes(block->padding, sizeof(block->padding)) ||
        !finite_float(block->channel_scale) ||
        !hp1::encode_carrier(block->m, result.carrier))
      return;
    result.column_scale = block->channel_scale;
    result.status = WeightReaderStatus::Success;
  };
  switch (args.weight_format) {
  case Format::q4_hp1:
    read(native_block(args.q4_hp1_blocks, args.native_block_count,
                      args.native_blocks_per_row, column, original_block));
    break;
  case Format::q8_hp1:
    read(args.q8_hp1_block(column, original_block));
    break;
  case Format::q16_hp1:
    read(native_block(args.q16_hp1_blocks, args.native_block_count,
                      args.native_blocks_per_row, column, original_block));
    break;
  default:
...
ColumnScaleResult
read_column_scale_validated(const ggml_gemmini_args_t &args,
                            const wroute::WeightRoutePlan &plan, size_t column,
                            size_t original_block) {
  if (plan.hp1_carriers) {
    const auto value =
        read_hp1_carrier_validated(args, plan, column, original_block);
    return {value.status, value.column_scale};
  }
  const auto value = read_scale_validated(args, plan, column, original_block);
  if (value.ok() && value.domain != ScaleDomain::IntegerBlockTimesColumn)
    return {WeightReaderStatus::InvalidMetadata, 0.0f};
  return {value.status, value.column_scale};
}
...
WeightReaderStatus validate(const ggml_gemmini_args_t &args,
  ...
  for (size_t j = 0; j < args.J; ++j) {
    for (size_t block = 0; block < plan.scales.cols; ++block) {
      if (plan.hp1_carriers) {
        const auto carrier = read_hp1_carrier_validated(args, plan, j, block);
        if (!carrier.ok())
          return carrier.status;
      } else {
        const WeightScaleResult scale =
            read_scale_unchecked(args, plan, j, block);
        if (!scale.ok())
          return scale.status;
      }
    }
...
WeightCodeResult read_code_validated(const ggml_gemmini_args_t &args,
  ...
  case Format::q4_h0: {
    const block_q4_h0 *block =
        native_block(args.q4_h0_blocks, args.native_block_count,
                     args.native_blocks_per_row, j, block_index);
    if (block != nullptr)
      result.value = decode_q4(block->qs, local_index);
    else
      result.status = WeightReaderStatus::InvalidMetadata;
    break;
  }
  ...
  case Format::q8_h1: {
    const block_q8_h1 *block = args.q8_h1_block(j, block_index);
    if (block != nullptr)
      result.value = block->qs[local_index];
    else
      result.status = WeightReaderStatus::InvalidMetadata;
    break;
  }
...
WeightReaderStatus read_code_tile_validated(const ggml_gemmini_args_t &args,
  ...
  if (local_k == nullptr || values == nullptr || valid_k == 0 ||
      valid_k > DIM || valid_k > kBlockSize || valid_cols == 0 ||
      valid_cols > DIM || col_base > args.J || valid_cols > args.J - col_base ||
  ...
  if (plan.route == Route::H1 || plan.route == Route::HP1) {
    switch (args.weight_format) {
    case Format::q4_h1:
      return gather([&](size_t j) {
        return native_block(args.q4_h1_blocks, args.native_block_count,
                            args.native_blocks_per_row, j, block_index);
      });
    case Format::q4_hp1:
      return gather([&](size_t j) {
        return native_block(args.q4_hp1_blocks, args.native_block_count,
                            args.native_blocks_per_row, j, block_index);
      });
    case Format::q8_h1:
    ...
    case Format::q8_hp1:
      return gather(
          [&](size_t j) { return args.q8_hp1_block(j, block_index); });
    case Format::q16_h1:
      return gather([&](size_t j) {
        return native_block(args.q16_h1_blocks, args.native_block_count,
                            args.native_blocks_per_row, j, block_index);
      });
    case Format::q16_hp1:
      return gather([&](size_t j) {
        return native_block(args.q16_hp1_blocks, args.native_block_count,
                            args.native_blocks_per_row, j, block_index);
      });
    default:
      break;
  ...
  for (size_t k = 0; k < valid_k; ++k) {
    for (size_t col = 0; col < valid_cols; ++col) {
      const WeightCodeResult code =
          read_code(args, plan, col_base + col, k_base + local_k[k]);
      if (!code.ok())
        return code.status;
      ++resolutions;
...
WeightScaleResult read_scale(const ggml_gemmini_args_t &args,
  ...
  return storage == WeightReaderStatus::Success
             ...
             : invalid_scale(storage);
}

const char *weight_reader_status_name(WeightReaderStatus status) {
  switch (status) {
  case WeightReaderStatus::Success:
    return "success";
  case WeightReaderStatus::InvalidArguments:
    return "invalid-arguments";
  case WeightReaderStatus::InvalidMetadata:
    return "invalid-metadata";
  case WeightReaderStatus::ScaleOverflow:
    return "scale-overflow";
  case WeightReaderStatus::UnsupportedFormat:
