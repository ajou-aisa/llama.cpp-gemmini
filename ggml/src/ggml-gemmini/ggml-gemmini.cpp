...
/* ______________________________________________________________ */
    ...
    fprintf(stderr, "[gemmini_tensor_cycles] start = %d, end = %d, elapsed = %d", start, end, end - start);
    before_gemmini_overhead_cycles += gemmini_tensor_cycles;
    fprintf(stderr, "[before_gemmini_overhead_cycles] = %d", before_gemmini_overhead_cycles);
    /* --------------------------------- Cycle -------------------------------- */
    ...
    fprintf(stderr, "[out_copy_cycles] start = %d, end = %d, elapsed = %d", start, end, end - start);

    after_gemmini_overhead_cycles += out_copy_cycles;
    /* --------------------------------- Cycle -------------------------------- */
    fprintf(stderr, "[after_gemmini_overhead_cycles] = %d", after_gemmini_overhead_cycles);
...
static enum ggml_status ggml_backend_gemmini_graph_compute(ggml_backend_t backend, struct ggml_cgraph * cgraph) {
    ...
    fprintf(stderr, "[bias_mapping_cycles] start = %d, end = %d, elapsed = %d", start, end, end - start);
    ...
    fprintf(stderr, "[tmp_ctx_cycles] start = %d, end = %d, elapsed = %d", start, end, end - start);
    /* --------------------------------- Cycle -------------------------------- */
...
/* _____________________________________________________________________________ */
    ...
    for (int i = 0; i < cgraph->n_nodes; i++)
    {
        ...
        case GGML_OP_MUL_MAT: {
            ...
            fprintf(stderr, "[bias_getting_cycles] start = %d, end = %d, elapsed = %d", start, end, end - start);
            /* --------------------------------- Cycle -------------------------------- */
