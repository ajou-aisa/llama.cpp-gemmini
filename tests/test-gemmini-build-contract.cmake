...
function(run_configure_case name stripe pipeline mode local_workers rmd block_size expect_success)
    ...
    if (ARGC GREATER 8)
        ...
        string(FIND "${output}" "${ARGV8}" failure_needle_at)
        if (failure_needle_at EQUAL -1)
            message(FATAL_ERROR
                "Expected configure failure for ${name} to mention '${ARGV8}'\nstdout:\n${stdout}\nstderr:\n${stderr}")
        endif()
    endif()
endfunction()
...
run_configure_case(full_with_features ON ON FULL 4 ON 32 TRUE)
run_configure_case(stripe_sequential_without_pipeline ON OFF STRIPE_SEQUENTIAL 4 ON 32 TRUE)
run_configure_case(stripe_pipeline_with_features ON ON STRIPE_PIPELINE 4 ON 32 TRUE)
run_configure_case(exsia_local_workers_three ON ON FULL 3 ON 32 TRUE)
run_configure_case(rmd_ablation ON ON FULL 4 OFF 32 TRUE)
run_configure_case(block_size_64 ON ON FULL 4 ON 64 TRUE)
run_configure_case(block_size_128 ON ON FULL 4 ON 128 TRUE)
run_configure_case(stripe_sequential_without_stripe OFF OFF STRIPE_SEQUENTIAL 4 ON 32 FALSE)
run_configure_case(stripe_pipeline_without_pipeline ON OFF STRIPE_PIPELINE 4 ON 32 FALSE)
run_configure_case(exsia_local_workers_two ON ON FULL 2 ON 32 FALSE)
