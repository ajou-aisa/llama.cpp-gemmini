...
BUILD_DIR=${BUILD_DIR:-build-arm64-cpu}
...
CYCLE_DETAIL_DEFAULT=${CYCLE_DETAIL:-1} # 0 | 1
...
if [[ -n "${BUILD_JOBS:-}" ]]; then
  BUILD_JOBS_DEFAULT=$BUILD_JOBS
elif [[ "$(uname -s)" == "Darwin" ]]; then
  BUILD_JOBS_DEFAULT=$(sysctl -n hw.logicalcpu)
elif command -v nproc >/dev/null 2>&1; then
  BUILD_JOBS_DEFAULT=$(nproc)
else
  BUILD_JOBS_DEFAULT=$(getconf _NPROCESSORS_ONLN)
fi
...
GGML_GEMMINI_EXECUTION_BACKEND_DEFAULT=${GGML_GEMMINI_EXECUTION_BACKEND:-HARDWARE} # HARDWARE | IM2P_SIM | FPGA_UART
