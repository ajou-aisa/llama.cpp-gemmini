#include "../ggml/src/ggml-gemmini/quants/common/hp1_scu.hpp"
...
bool matches_division_oracle(int32_t residual,
                             const NativeBalancedDigits &digits) {
    int64_t remaining = residual;
    for (size_t lane = 0; lane < digits.digits.size(); ++lane) {
        int64_t digit = remaining % digits.radix;
    if (digit >= static_cast<int64_t>(digits.radix / 2))
      digit -= digits.radix;
    if (digit < -static_cast<int64_t>(digits.radix / 2))
      digit += digits.radix;
    if (digits.digits[lane] != digit)
      return false;
...
bool test_width_native_radix_happy_boundaries() {
  ...
  bool ok = check(
      decompose_balanced_radix(0, 8, zero) == RmdStatus::success &&
  ...
  for (const WidthCase &width : widths) {
    ...
    ok = check(contract.radix == width.radix &&
                   contract.lane_capacity == width.capacity &&
                       contract.digit_min == width.digit_min &&
                       contract.digit_max == width.digit_max,
               "width selects native radix, digit range, and lane capacity") &&
         ok;
        for (const int32_t value : values) {
            NativeBalancedDigits digits{};
      const RmdStatus status =
          decompose_balanced_radix(value, width.bits, digits);
      bool value_ok =
          check(status == RmdStatus::success, "full int32 boundary decomposes");
            value_ok = check(digits.radix == width.radix &&
                                 digits.lane_capacity == width.capacity &&
                                 digits.active_lane_count <= width.capacity,
                       "decomposition records the selected width contract") &&
                 value_ok;
            for (uint8_t lane = 0; lane < width.capacity; ++lane) {
                value_ok = check(digits.digits[lane] >= width.digit_min &&
                         ...
                         "every native digit stays in its signed range") &&
                   value_ok;
            }
      for (uint8_t lane = digits.active_lane_count; lane < width.capacity;
           ++lane) {
                value_ok = check(digits.digits[lane] == 0,
                         "inactive trailing lanes are zero") &&
                   value_ok;
            }
            ...
            value_ok = check(digits.active_lane_count == expected_active,
                       "active lanes trim only trailing zero places") &&
                 value_ok;
      value_ok =
          check(matches_division_oracle(value, digits),
                "independent division oracle agrees on every signed digit") &&
          value_ok;
            int64_t composed = std::numeric_limits<int64_t>::min();
      value_ok = check(compose_balanced_radix(digits, composed) ==
                               RmdStatus::success &&
                                 composed == value,
                       "production recomposition matches the boundary") &&
                 value_ok;
            ok = value_ok && ok;
        }
            ...
            for (const int64_t candidate : {
               ...
               -boundary - 1,
                ...
                ok = check(decompose_balanced_radix(static_cast<int32_t>(candidate),
                       ...
                       matches_division_oracle(static_cast<int32_t>(candidate),
                                               digits),
                   "every native digit boundary carries and borrows exactly") &&
             ok;
            }
    ...
    for (int64_t candidate :
         {without_top_carry_max - 1, without_top_carry_max,
      ...
      ok = check(decompose_balanced_radix(static_cast<int32_t>(candidate),
                 ...
                 "first top carry uses the extra ordinary radix lane") &&
           ok;
        }
      ...
      const int64_t signed_value =
          state <= uint32_t{INT32_MAX}
              ? static_cast<int64_t>(state)
              : static_cast<int64_t>(state) - (int64_t{1} << 32);
            NativeBalancedDigits digits{};
      if (!check(decompose_balanced_radix(static_cast<int32_t>(signed_value),
                                          width.bits,
                                                digits) == RmdStatus::success &&
                     matches_division_oracle(static_cast<int32_t>(signed_value),
                                             digits),
                 "full-range samples match independent division without signed "
                 "overflow")) {
                return false;
            }
        }
...
bool test_width_native_compose_and_expand() {
  ...
  for (const WidthCase &test : cases) {
    ...
    const RmdStatus status =
        compose_rmd_output(*packet, compressed, correction);
    const auto *values = std::get_if<BlockScaledInt64Correction>(&correction);
    ok = check(
             status == RmdStatus::success && values != nullptr &&
                       values->values.size() == weights.size() &&
                 values->values[0] == static_cast<int64_t>(test.residual) *
                                          weights[0] * block_scale &&
                 values->values[1] == static_cast<int64_t>(test.residual) *
                                          weights[1] * block_scale,
             "checked radix composition matches independent residual oracle") &&
         ok;

        std::vector<int32_t> plane;
        expand_packets_to_plane({packet}, 1, 1, plane);
        ok = check(plane.size() == 1 && plane[0] == test.residual,
               "typed packet expansion reconstructs the original residual") &&
         ok;
        ...
        {std::make_shared<const StripePacket>(std::move(malformed_packet))}, 1,
        1, plane);
        ok = check(plane == std::vector<int32_t>({121, 123}),
               "malformed typed expansion leaves caller plane unchanged") &&
         ok;
        ...
        correction = sentinel;
    const auto malformed_status =
        compose_rmd_output(*packet, malformed, correction);
    const auto *malformed_values =
        ...
        ok = check(malformed_status == RmdStatus::invalid_arguments &&
               ...
               "malformed compressed geometry leaves correction unchanged") &&
         ok;
    }
    ...
    const auto *cancellation_values =
            std::get_if<BlockScaledInt64Correction>(&correction);
    ok =
        check(cancellation_status == RmdStatus::success &&
                       cancellation_values != nullptr &&
                       cancellation_values->values == std::vector<int64_t>({1}),
              "wide radix contribution cancels before final int64 narrowing") &&
        ok;

        for (uint8_t lane_position = 0; lane_position < 2; ++lane_position) {
            compressed.values[block.output_value_offset +
                        static_cast<size_t>(lane_position) *
                            block.lane_stride_values] =
    ...
    const auto *overflow_values =
        ...
        ok = check(overflow_status == RmdStatus::overflow &&
                       ...
                       overflow_values->values ==
                           std::get<BlockScaledInt64Correction>(sentinel).values,
               "native-radix overflow leaves correction unchanged") &&
         ok;

        builder.reset(38, 0, 1, kBlockSize + 1, 1, bits);
    if (!check(
            builder.add_residual(0, 0, static_cast<int32_t>(contract.radix)) &&
    ...
    if (!check(cancellation_packet != nullptr &&
                   cancellation_packet->blocks.size() == 2,
    ...
    compressed.values[cancellation_packet->blocks[0].output_value_offset] =
        int64_t{1} << 62;
    compressed.values[cancellation_packet->blocks[1].output_value_offset] =
        -(int64_t{1} << 62);
        correction = sentinel;
    ...
    const auto *block_cancellation_values =
            std::get_if<BlockScaledInt64Correction>(&correction);
    ok = check(
             block_cancellation_status == RmdStatus::success &&
                       block_cancellation_values != nullptr &&
                       block_cancellation_values->values == std::vector<int64_t>({0}),
             "wide block contributions cancel before final int64 narrowing") &&
         ok;
    }
    return ok;
}
    ...
    for (const uint8_t bits : widths) {
        ...
        for (const int32_t top_digit : {-1, 1}) {
            ...
            ok = check(compose_balanced_radix(digits, output) ==
                           ...
                           output == 123456789,
                 "digits representing plus or minus 2^32 reject atomically") &&
           ok;
        }
    }
    return ok;
...
bool test_width_native_malformed_compose_is_atomic() {
    constexpr std::array<uint8_t, 3> widths = {4, 8, 16};
    bool ok = true;
  auto rejects_without_output_mutation =
      [&](const NativeBalancedDigits &malformed, const char *message) {
    ...
    if (!check(decompose_balanced_radix(1, bits, canonical) ==
                   RmdStatus::success,
        ...
        ok = rejects_without_output_mutation(
             untrimmed, "untrimmed active-lane metadata rejects atomically") &&
         ok;
        ...
        ok = rejects_without_output_mutation(
             missing_active,
             "nonzero logical digit requires active-lane metadata") &&
         ok;

        NativeBalancedDigits zero_untrimmed{};
    if (!check(decompose_balanced_radix(0, bits, zero_untrimmed) ==
                   RmdStatus::success,
    ...
    ok =
        rejects_without_output_mutation(
            zero_untrimmed, "zero decomposition cannot claim an active lane") &&
        ok;
        ...
        if (canonical.lane_capacity < canonical.digits.size()) {
            ...
            ok = rejects_without_output_mutation(
               physical_tail, "nonzero physical tail beyond logical capacity "
                              "rejects atomically") &&
           ok;
        }
    }
...
bool test_top_carry_lane_preserves_sparse_mask() {
    ...
    for (uint8_t bits : {uint8_t{4}, uint8_t{8}, uint8_t{16}}) {
        ...
        if (!check(builder.add_residual(0, 31, std::numeric_limits<int32_t>::max()),
               "builder accepts full int32 residual at K31"))
      return false;
        const auto packet = builder.finish();
        if (!check(packet != nullptr && packet->blocks.size() == 1,
               "top carry packet builds"))
      return false;
    const auto &block = packet->blocks.front();
        const uint8_t top_lane = 32 / bits;
    const uint16_t expected_mask =
        (1u << 0) | (1u << (top_lane - 1)) | (1u << top_lane);
    ok = check(block.active_lane_mask == expected_mask &&
                   block.active_lane_count == 3 && block.lane_ids[0] == 0 &&
                   block.lane_ids[1] == top_lane - 1 &&
                       block.lane_ids[2] == top_lane &&
                       block.lane_k_masks[top_lane] == (uint32_t{1} << 31) &&
                       validate_packet(*packet) == RmdStatus::success,
               "sparse mask retains original top lane and original K31") &&
         ok;
    }
    return ok;
}
...
bool test_EXPLICIT_BLOCK_ID_DIM_PADDING() {
  ...
  if (!builder.add_residual(0, 31, 1) || !builder.add_residual(1, 32, 65536) ||
  ...
  const BlockDescriptor &second = packet->blocks[1];
    bool ok = true;
  ok = check(first.block_id == 0 && first.global_k_begin == 0,
             "first block id") &&
       ok;
  ok = check(second.block_id == 1 && second.global_k_begin == kBlockSize,
             "second block id") &&
       ok;
    ok = check(second.active_lane_mask == 0x05 && second.active_lane_count == 2,
             "lane gap remains sparse") &&
       ok;
    ok = check(second.lane_ids[0] == 0 && second.lane_ids[1] == 2,
             "lane ids preserve radix places") &&
       ok;
    ok = check(first.rows_padded == kArrayDim && second.rows_padded == kArrayDim,
             "rows use DIM padding") &&
       ok;
  ok = check(first.padded_k_count == kArrayDim &&
                 second.padded_k_count == kArrayDim,
             "each block uses independent DIM padding") &&
       ok;
    ok = check(packet->j_padded == align_up(packet->logical_j, kArrayDim),
             "output columns use DIM padding") &&
       ok;
  ok = check(validate_packet(*packet) == RmdStatus::success,
             "built packet validates") &&
       ok;

    StripePacket malformed = *packet;
  malformed.stacked_activation
      .signed_int8[first.activation_offset + first.compact_k_count] = 1;
    ok = check(validate_packet(malformed) == RmdStatus::invalid_packet,
             "nonzero K padding rejected") &&
       ok;
    malformed = *packet;
  malformed.stacked_activation
      .signed_int8[first.activation_offset +
                   packet->row_count * first.padded_k_count] = 1;
    return check(validate_packet(malformed) == RmdStatus::invalid_packet,
               "nonzero row padding rejected") &&
         ok;
}
...
bool test_empty_residual_is_empty_success() {
    ...
    return check(builder.add_residual(0, 0, 0), "zero residual accepted") &&
        ...
        check(builder.finish() == nullptr, "empty residual emits no packet") &&
         check(builder.status() == RmdStatus::success,
               "empty residual is successful");
}
...
bool test_padding_overflow_fails() {
  ...
  return check(builder.finish() == nullptr,
               "padding overflow emits no packet") &&
         check(builder.status() == RmdStatus::overflow,
               "padding overflow is explicit");
}
...
bool test_weight_capability_happy_table() {
    ...
    constexpr std::array<CapabilityCase, 9> cases = {{
      {4, WeightFamily::H0, wroute::WeightRouteKind::H0,
       wroute::WeightScaleDomain::FloatingBlock, false},
      {4, WeightFamily::H1, wroute::WeightRouteKind::H1,
       wroute::WeightScaleDomain::IntegerBlockTimesColumn, true},
      {4, WeightFamily::HP1, wroute::WeightRouteKind::HP1,
       wroute::WeightScaleDomain::IntegerBlockTimesColumn, true},
      {8, WeightFamily::H0, wroute::WeightRouteKind::H0,
       wroute::WeightScaleDomain::FloatingBlock, false},
      {8, WeightFamily::H1, wroute::WeightRouteKind::H1,
       wroute::WeightScaleDomain::IntegerBlockTimesColumn, true},
      {8, WeightFamily::HP1, wroute::WeightRouteKind::HP1,
       wroute::WeightScaleDomain::IntegerBlockTimesColumn, true},
      {16, WeightFamily::H0, wroute::WeightRouteKind::H0,
       wroute::WeightScaleDomain::FloatingBlock, false},
      {16, WeightFamily::H1, wroute::WeightRouteKind::H1,
       wroute::WeightScaleDomain::IntegerBlockTimesColumn, true},
      {16, WeightFamily::HP1, wroute::WeightRouteKind::HP1,
       wroute::WeightScaleDomain::IntegerBlockTimesColumn, true},
    }};
  ...
  for (const CapabilityCase &test : cases) {
    ...
    const wreader::WeightCodeResult zero =
        wreader::read_code(fixture.args, plan, 0, 16);
        ok = check(plan.valid && plan.route == test.route &&
                   plan.weight_bits == test.bits &&
                   plan.scale_domain == test.domain,
               "residual route capability resolves by family and width") &&
         ok;
    ok = check(zero.status == wreader::WeightReaderStatus::Success &&
                   zero.value == 0,
               "RMD capability uses the shared signed-code reader") &&
         ok;
    ok = check(wroute::weight_route_status(
                   plan, wroute::WeightExecutionPath::CpuDirect) ==
                       wroute::WeightRouteStatus::Success,
               "all supported residual families allow CPU-direct") &&
         ok;
    ok = check(wroute::weight_route_status(
                   plan, wroute::WeightExecutionPath::Compact) ==
                   (test.compact
                        ? wroute::WeightRouteStatus::Success
                        : wroute::WeightRouteStatus::UnsupportedExecution),
               "compact capability follows the scale domain") &&
         ok;
    }
    return ok;
}
...
bool test_weight_capability_failure_table() {
    ...
    ok = check(wroute::weight_route_status(
                   plan, wroute::WeightExecutionPath::Compact) ==
                       wroute::WeightRouteStatus::UnsupportedExecution,
               "H0 compact request is rejected before packet work") &&
         ok;
    }
  ...
  ok = check(wroute::weight_route_status(
                 invalid, wroute::WeightExecutionPath::CpuDirect) ==
                   wroute::WeightRouteStatus::InvalidMetadata,
             "invalid metadata remains a typed route failure") &&
       ok;
    return ok;
}
...
struct DirectOracleFixture {
    ...
    auto populate_q4 = [](auto &blocks) {
      ...
      blocks[0].qs[0] = 0xf0;
      blocks[0].qs[15] = 0x78; // [-8, +7, -1]
      blocks[1].qs[0] = 0x8f;
      blocks[1].qs[15] = 0xe0; // [+7, -8, +6]
      blocks[2].qs[0] = 0x0f;
      blocks[2].qs[15] = 0xb8; // [+7, -8, +3]
      blocks[3].qs[0] = 0x80;
      blocks[3].qs[15] = 0x4f; // [-8, +7, -4]
        };
        populate_q4(q4_h0);
        populate_q4(q4_h1);
        populate_q4(q4_hp1);
    ...
    auto populate_q16 = [](auto &blocks) {
      blocks[0].qs[0] = -32768;
      blocks[0].qs[16] = 32767;
      blocks[0].qs[31] = -257;
      blocks[1].qs[0] = 32767;
      blocks[1].qs[15] = -32768;
      blocks[1].qs[31] = 12345;
      blocks[2].qs[0] = 32767;
      blocks[2].qs[16] = -32768;
      blocks[2].qs[31] = 511;
      blocks[3].qs[0] = -32768;
      blocks[3].qs[15] = 32767;
      blocks[3].qs[31] = -16384;
        };
        populate_q16(q16_h0);
        populate_q16(q16_h1);
    ...
    constexpr std::array<float, 4> hp1_columns = {0.125f, 0.125f, -0.75f,
                                                  -0.75f};
    ...
    for (const residual::ResidualEvent &event : events) {
      if (!builder.add_residual(event.local_row, event.original_k,
                                event.residual)) {
                return;
            }
        }
...
constexpr std::array<DirectOracleCase, 9> kDirectOracleCases = {{
    ...
    {16,
     WeightFamily::H0,
     {},
     {-82561.5, 709500.0, 0.0, 0.0, 213375.5, -383109.0}},
    {16, WeightFamily::H1, {-330246, 2792901, 0, 0, 853502, -3576259}, {}},
    {16, WeightFamily::HP1, {-330246, 4335528, 0, 0, 853502, -5380008}, {}},
}};

template <typename T, size_t N>
bool values_match(const std::vector<T> &actual,
                  const std::array<T, N> &expected) {
    return actual.size() == expected.size() &&
        std::equal(actual.begin(), actual.end(), expected.begin());
}

bool direct_outputs_match(const rmd::DirectOutput &lhs,
                          const rmd::DirectOutput &rhs) {
...
bool test_block_direct_weight_families() {
    bool ok = true;
  for (WeightFamily family :
       {WeightFamily::H0, WeightFamily::H1, WeightFamily::HP1}) {
        DirectOracleFixture fixture(8, family);
    if (!check(fixture.payload != nullptr,
               "BLOCK direct family fixture builds")) {
            return false;
        }
    ...
    std::vector<double> expected(fixture.payload->row_count * fixture.args.J,
                                 0.0);
    for (const residual::ResidualEvent &event : fixture.payload->events) {
          ...
          const auto &weight = fixture.q8_hp1[index];
                    code = weight.qs[k];
          scale = weight.m == std::numeric_limits<int16_t>::min()
                      ? 0.0
                      : std::ldexp(static_cast<double>(weight.channel_scale),
                                   weight.m);
                }
                expected[event.local_row * fixture.args.J + j] +=
            static_cast<double>(event.residual) * static_cast<double>(code) *
    ...
    const RmdStatus status =
        residual::execute_direct_stripe(fixture.args, *fixture.payload, actual);
    const auto *fully_scaled =
        std::get_if<FullyScaledFloat64Correction>(&actual);
    ok =
        check(
            status == RmdStatus::success && fully_scaled != nullptr &&
                       fully_scaled->values == expected,
            "BLOCK H0/H1/HP1 direct matches independent per-block reference") &&
        ok;
    }
  if (ok)
    std::puts("BLOCK_DIRECT_FAMILIES H0=pass H1=pass HP1=pass scales=differ");
    return ok;
}
    ...
    const rmd::RmdStatus status =
      ...
      case_ok =
          check(output != nullptr &&
                ...
                "H1/HP1 direct output matches literal integer-domain oracle") &&
          case_ok;
            if (output != nullptr) {
                std::vector<rmd::ReferenceResidual> residuals;
                residuals.reserve(fixture.payload->events.size());
        for (const residual::ResidualEvent &event : fixture.payload->events) {
          residuals.push_back({static_cast<uint32_t>(event.local_row),
                ...
                const rmd::RmdStatus radix_status = rmd::reference_rmd_correction(
                    ...
                    radix_reference);
        case_ok =
            check(direct_status == rmd::RmdStatus::success &&
        ...
        case_ok =
            check(
                tagged_status == rmd::RmdStatus::success &&
                    tagged_integer != nullptr &&
                        values_match(tagged_integer->values, test.integer_expected),
                "Q8 integer direct output retains its explicit domain tag") &&
            case_ok;
            }
        }
        case_ok = check(metrics.event_count == fixture.payload->events.size() &&
                            metrics.call_count == 1,
                    "direct metrics commit only after complete success") &&
              case_ok;
        std::printf("DIRECT_ORACLE width=%u family=%s status=%s values=6\n",
                static_cast<unsigned>(test.bits),
                weight_family_name(test.family),
...
bool test_a16_allocation_products() {
  ...
  if (!check(buffer.allocate(1, 2, 16),
             "A16 allocation sentinel initializes")) {
        return false;
    }
  ...
  ok =
      check(!buffer.allocate(std::numeric_limits<size_t>::max() / 4 + 1, 2, 16),
            "A16 row byte product overflow rejects before allocation") &&
      ok;
    return check(buffer.bytes == original_bytes && buffer.bytes->at(0) == 0x5a &&
                   buffer.rows == original_rows &&
                   buffer.cols == original_cols &&
                     buffer.row_stride_bytes == original_stride,
               "failed A16 allocation leaves prior buffer byte-identical") &&
         ok;
}

bool test_direct_failure_matrix() {
  const rmd::DirectOutput sentinel =
      rmd::PreScaledFloat64Correction{{13.25, -9.5, 4.0}};
  auto fails_atomically = [&](const ggml_gemmini_args_t &args,
                              const residual::DirectStripePayload &payload,
                              rmd::RmdStatus expected, const char *message) {
  ...
  residual::DirectStripePayload malformed_payload =
      *malformed_payload_fixture.payload;
    ...
    ok = fails_atomically(malformed_payload_fixture.args, malformed_payload,
                          rmd::RmdStatus::invalid_packet,
                        "malformed payload leaves direct output unchanged") &&
       ok;
    ...
    ok = fails_atomically(missing_extent.args, *missing_extent.payload,
                        ...
                        "zero native byte extent rejects atomically") &&
       ok;

    DirectOracleFixture malformed_storage(4, WeightFamily::H1);
  malformed_storage.args.native_weight_bytes =
      sizeof(malformed_storage.q4_h1) - 1;
  ok =
      fails_atomically(malformed_storage.args, *malformed_storage.payload,
                          rmd::RmdStatus::unsupported_route,
                       "truncated Q4 storage leaves direct output unchanged") &&
      ok;
    ...
    ok = check(rmd::reference_rmd_correction(
             ...
             "malformed width-native reference input rejects atomically") &&
       ok;
  ...
  ok = fails_atomically(
           invalid_h0.args, *invalid_h0.payload,
                          rmd::RmdStatus::unsupported_route,
           "non-finite H0 block scale leaves direct output unchanged") &&
       ok;
  ...
  ok =
      fails_atomically(shape_overflow.args, oversized, rmd::RmdStatus::overflow,
                       "direct result size overflow leaves output unchanged") &&
      ok;
  ...
  ok = check(scale_payload != nullptr,
             "integer scale overflow payload builds") &&
       ok;
    if (scale_payload != nullptr) {
    ok = fails_atomically(scale_overflow.args, *scale_payload,
                          rmd::RmdStatus::overflow,
                          "int64 block-scale product overflow is atomic") &&
         ok;
    }
  ...
  ok = check(add_payload != nullptr, "integer add overflow payload builds") &&
       ok;
    if (add_payload != nullptr) {
        ok = fails_atomically(add_args, *add_payload, rmd::RmdStatus::overflow,
                          "int64 cross-block add overflow is atomic") &&
         ok;
    }

    if (ok) {
    std::puts("DIRECT_FAILURE_MATRIX malformed=1 storage=1 a16=1 scale=1 add=1 "
...
struct CompactOracleFixture {
    ...
    auto set_q4 = [](auto &block, size_t local_k, int32_t value) {
        ...
        block.qs[byte] =
            static_cast<uint8_t>((block.qs[byte] & 0xf0u) | nibble);
            } else {
        block.qs[byte] =
            static_cast<uint8_t>((block.qs[byte] & 0x0fu) | (nibble << 4));
            }
        };
        for (size_t j = 0; j < columns; ++j) {
          ...
          int32_t code =
              bits == 4   ? static_cast<int32_t>(seed % 16) - 8
      ...
      const uint8_t code =
          static_cast<uint8_t>(std::min<uint64_t>(h1_scale, 255));
      ...
      integer_scales[index] =
          family == WeightFamily::H1 ? h1_scale
          : hp1_exponents[index] == std::numeric_limits<int16_t>::min()
              ? 0
      ...
      places[active_lane_count] = places[active_lane_count - 1] * radix.radix;
            ++active_lane_count;
        }
        auto digit_for = [&](size_t ordinal, uint8_t lane) {
      return places[lane] >= fixture_magnitude || ordinal % 2 != 0 ? int8_t{-1}
                                                                   : int8_t{1};
        };
  ...
  int32_t code(size_t j, size_t k) const { return codes[j * logical_k + k]; }
};
...
bool compact_oracle_output(const CompactOracleFixture &fixture,
  ...
  for (const BlockDescriptor &block : fixture.packet->blocks) {
      ...
      const size_t lane_base =
          block.output_value_offset + lane_position * block.lane_stride_values;
            for (size_t row = 0; row < CompactOracleFixture::rows; ++row) {
                for (size_t j = 0; j < CompactOracleFixture::columns; ++j) {
          using Format = ggml_gemmini_args_t::im2p_weight_format_t;
          const bool hp1 = fixture.args.weight_format == Format::q4_hp1 ||
                           fixture.args.weight_format == Format::q8_hp1;
          if (hp1) {
            namespace wr = ggml::gemmini::quants::wreader;
            namespace route = ggml::gemmini::quants::wroute;
            namespace scu = ggml::gemmini::quants::hp1;
            const auto plan = route::resolve_weight_route_plan(
                fixture.args, route::WeightScaleInfoMode::ResidualHp1Scu);
            const auto carrier = wr::read_hp1_carrier_validated(
                fixture.args, plan, j, block.block_id);
            if (!plan.valid || !carrier.ok())
              return false;
            const auto group = std::find_if(
                block.groups.begin(), block.groups.end(), [&](const auto &g) {
                  return std::find(g.lane_positions.begin(),
                                   g.lane_positions.end(),
                                   lane_position) != g.lane_positions.end();
                });
            if (group == block.groups.end())
              return false;
            std::vector<size_t> selected;
            for (size_t compact = 0; compact < block.compact_k_count; ++compact)
              if (group->k_mask &
                  (uint32_t{1}
                   << fixture.packet
                          ->k_indices[block.k_index_offset + compact]))
                selected.push_back(compact);
            int32_t acc = 0;
            for (size_t begin = 0; begin < selected.size();
                 begin += std::min<size_t>(kArrayDim, 32)) {
              int64_t raw_fragment = 0;
              for (size_t q = begin;
                   q < std::min(selected.size(),
                                begin + std::min<size_t>(kArrayDim, 32));
                   ++q) {
                int32_t digit = 0;
                const auto compact = selected[q];
                if (read_packet_digit(*fixture.packet, block, lane_position,
                                      row, compact,
                                      digit) != RmdStatus::success)
                  return false;
                const size_t original_k =
                    block.global_k_begin +
                    fixture.packet->k_indices[block.k_index_offset + compact];
                raw_fragment += int64_t(digit) * fixture.code(j, original_k);
              }
              if (raw_fragment < INT32_MIN || raw_fragment > INT32_MAX)
                return false;
              const auto scaled = scu::apply_validated(
                  static_cast<int32_t>(raw_fragment), carrier.carrier);
              acc = begin == 0 ? scaled : scu::accumulate(acc, scaled);
            }
            expected[lane_base + row * fixture.packet->j_padded + j] = acc;
            continue;
          }
          // Explicit legacy H1/A16 oracle; not a production HP1 SCU model.
                    __int128 raw = 0;
          for (const CompactOracleResidual &residual : fixture.residuals) {
            if (residual.event.local_row != row ||
                residual.lane_id != lane_id ||
          ...
          const size_t scale_index =
              j * CompactOracleFixture::blocks_per_row + block.block_id;
...
bool test_block_scale_before_sum() {
  ...
  if (!check(fixture.valid, "BLOCK compact scale fixture builds"))
    return false;
    ...
    if (!check(packet != nullptr && packet->blocks.size() == 2,
             "BLOCK compact packet keeps both original K blocks"))
    return false;
  ...
  auto &metadata = fixture.args.act_quant.storage().emplace<act::block::Meta>();
    metadata.rows = 1;
  ...
  const RmdStatus merge_status =
      compose_status == RmdStatus::success
          ? merge_rmd_correction(fixture.args, *packet, correction,
                                 &nonzero_count)
          : compose_status;
    ...
    residual::DirectExecutionMetrics metrics{};
  const RmdStatus direct_status =
      direct_payload == nullptr
          ? RmdStatus::invalid_packet
          : residual::execute_direct_stripe(fixture.args, *direct_payload,
                                            direct, &metrics);
  bool ok = check(
      merge_status == RmdStatus::success && destination[0] == -165.0f &&
          nonzero_count == 1 && metrics.event_count == 2 &&
          metrics.call_count == 1 && compact_status == RmdStatus::success &&
    ...
    destination[0] = 0.0f;
  ok = check(merge_rmd_correction(fixture.args, 0, 1, correction) ==
                     RmdStatus::success &&
                   destination[0] == static_cast<float>(3000000000.0),
             "fully-scaled correction is not clamped to INT32") &&
       ok;
  if (ok)
    std::puts("BLOCK_DIRECT_COMPACT correction=-288 merged=-165 unclamped=3e9 "
              "tag=fully_scaled");
    return ok;
}
...
bool test_block_correction_failure_atomicity() {
    CompactOracleFixture fixture(8, WeightFamily::H1);
  if (!check(fixture.valid, "BLOCK correction failure fixture builds"))
    return false;

  auto &metadata = fixture.args.act_quant.storage().emplace<act::block::Meta>();
    metadata.rows = fixture.args.I + fixture.packet->row_begin;
    metadata.cols = fixture.args.K;
  metadata.scales.assign(metadata.rows * CompactOracleFixture::blocks_per_row,
                         1.0f);
  ...
  bool ok = check(
      execute_rmd_stripe_ws(fixture.args, *fixture.packet, correction,
  ...
  if (!compact_oracle_output(fixture, values))
    return false;
    CompressedOutput compressed;
  ...
  ok = check(compose_block_rmd_output(fixture.args, *fixture.packet, compressed,
                                      correction) ==
                       RmdStatus::invalid_arguments &&
                   direct_outputs_match(correction, correction_before),
             "malformed BLOCK scale preserves compact correction") &&
       ok;
    ...
    ok = check(channel_args.has_q8_channel_dense_sidecar_contract() &&
                 compose_block_rmd_output(channel_args, *fixture.packet,
                                          compressed, correction) ==
                     RmdStatus::unsupported_route &&
                   direct_outputs_match(correction, correction_before) &&
                 execute_rmd_stripe_reference(channel_args, *fixture.packet,
                                              correction, &metrics) ==
                     RmdStatus::unsupported_route &&
                   direct_outputs_match(correction, correction_before) &&
                 metrics.packet_call_count == 17 &&
                 metrics.matmul_call_count == 19,
             "unsupported BLOCK channel correction preserves output and "
             "metrics") &&
       ok;
#endif
  ...
  ok = check(merge_rmd_correction(fixture.args, 0, 1, huge, &nonzero_count) ==
                     RmdStatus::overflow &&
                   destination == destination_before && nonzero_count == 29,
             "fully-scaled overflow preserves destination and metrics") &&
       ok;

    fixture.args.act_quant.storage().emplace<act::tensor::Meta>().scale = 1.0f;
  correction =
      FullyScaledFloat64Correction{std::vector<double>(fixture.args.J, 1.0)};
    destination.assign(destination.size(), 31.0f);
    const auto domain_before = destination;
    nonzero_count = 37;
  const bool domain_ok =
      check(merge_rmd_correction(fixture.args, 0, 1, correction,
                                 &nonzero_count) ==
                         RmdStatus::unsupported_route &&
                     destination == domain_before && nonzero_count == 37,
            "fully-scaled domain mismatch preserves destination and metrics") &&
      ok;
    if (domain_ok) {
    std::puts("BLOCK_CORRECTION_FAILURE malformed=atomic overflow=atomic "
              "domain=atomic host_fallback=rejected");
    }
    return domain_ok;
}
...
bool test_streaming_assembler_boundaries() {
    ...
    for (uint8_t bits : {uint8_t{4}, uint8_t{8}, uint8_t{16}}) {
        ...
        if (!check(packet != nullptr && packet->blocks.size() == 3,
               "streaming cancellation packet builds"))
      return false;
        CompressedOutput compressed;
        compressed.j_padded = packet->j_padded;
        compressed.values.assign(packet->total_output_values, 0);
    ...
    ok = check(compose_rmd_output(*packet, compressed, composed) ==
                       RmdStatus::success &&
               ...
               "streaming original lane shifts preserve wide cross-block "
               "cancellation") &&
         ok;
    ...
    ok =
        check(
            ...
            "streaming final narrowing overflow leaves correction unchanged") &&
        ok;
        ...
        ok = check(assembler.begin(*packet, streamed) == RmdStatus::success &&
                       ...
                       direct_outputs_match(streamed, sentinel),
               "streaming missing tiles do not publish a partial correction") &&
         ok;
        ok = check(assembler.begin(*packet, streamed) == RmdStatus::success &&
                       assembler.submit(tile) == RmdStatus::success &&
                       assembler.submit(tile) == RmdStatus::invalid_arguments &&
                       direct_outputs_match(streamed, sentinel),
               "streaming duplicate tile preserves correction") &&
         ok;
        ++tile.lane_id;
        ok = check(assembler.begin(*packet, streamed) == RmdStatus::success &&
                       assembler.submit(tile) == RmdStatus::invalid_arguments &&
                       direct_outputs_match(streamed, sentinel),
               "streaming mismatched lane tag preserves correction") &&
         ok;
    }
    return ok;
}
...
bool test_compact_oracle_happy_matrix() {
  ...
  for (const CompactCase &test : cases) {
    ...
    const RmdStatus gather_status =
        fixture.valid ? rmd::gather_wide_weight_tile_for_test(
                fixture.args, 1, edge_k.data(), edge_k.size(), 1,
                            CompactOracleFixture::columns - 1, wide_tile.data(),
                            kArrayDim, &gather_metrics)
                      : RmdStatus::invalid_packet;
    bool gather_ok =
        check(gather_status == RmdStatus::success,
                               "matched-width WeightGather accepts signed edge codes");
    gather_ok =
        check(
            gather_metrics.weight_values_gathered == edge_k.size() * 2 &&
                gather_metrics.weight_baseline_address_resolutions ==
                    edge_k.size() * 2 &&
                              gather_metrics.weight_address_resolutions == 2 &&
                ggml::gemmini::quants::wreader::
                        test_weight_reader_code_address_resolutions() == 2,
            "native gather resolves each selected column block exactly once") &&
        gather_ok;
        for (size_t k = 0; k < kArrayDim; ++k) {
            for (size_t j = 0; j < kArrayDim; ++j) {
        const int32_t expected_code =
            k < edge_k.size() && j < 2
                ? fixture.code(j + 1, kBlockSize + edge_k[k])
                : 123456789;
                gather_ok = check(wide_tile[k * kArrayDim + j] == expected_code,
                          "native gather preserves original K order, column "
                          "stride, and padding") &&
                    gather_ok;
            }
        }
        ...
        ggml::gemmini::quants::wreader::test_reset_weight_reader_counters();
    gather_ok =
        check(rmd::gather_wide_weight_tile_for_test(
                              fixture.args, 1, invalid_k.data(), invalid_k.size(), 1, 2,
                  wide_tile.data(), kArrayDim,
                  &gather_metrics) == RmdStatus::execution_failed &&
                  wide_tile == valid_tile &&
                  gather_metrics.weight_address_resolutions == 2 &&
                              gather_metrics.weight_values_gathered == edge_k.size() * 2 &&
                  ggml::gemmini::quants::wreader::
                          test_weight_reader_code_address_resolutions() == 0,
              "late invalid K rejects before block resolution without "
              "publishing a tile") &&
        gather_ok;
    ...
    gather_ok =
        check(rmd::gather_wide_weight_tile_for_test(
                  fixture.args, 1, &single_k, 1, 1, 2, wide_tile.data(),
                  kArrayDim, &gather_metrics) == RmdStatus::success &&
                  wide_tile == single_expected &&
                              gather_metrics.weight_values_gathered == 2 &&
                              gather_metrics.weight_baseline_address_resolutions == 2 &&
                              gather_metrics.weight_address_resolutions == 2 &&
                  ggml::gemmini::quants::wreader::
                          test_weight_reader_code_address_resolutions() == 2,
              "single-K native gather preserves K31, padding, and observed "
              "address counts") &&
        gather_ok;
        single_k = kBlockSize;
        ggml::gemmini::quants::wreader::test_reset_weight_reader_counters();
    gather_ok =
        check(rmd::gather_wide_weight_tile_for_test(
                  fixture.args, 1, &single_k, 1, 1, 2, wide_tile.data(),
                  kArrayDim, &gather_metrics) == RmdStatus::execution_failed &&
                  wide_tile == single_expected &&
                              gather_metrics.weight_values_gathered == 2 &&
                              gather_metrics.weight_address_resolutions == 2 &&
                  ggml::gemmini::quants::wreader::
                          test_weight_reader_code_address_resolutions() == 0,
              "invalid single local K preserves tile and metrics before any "
              "read") &&
        gather_ok;
#else
    bool gather_ok =
        check(false, "compact oracle requires GGML_GEMMINI_TESTING");
#endif
        ...
        RmdExecutionMetrics metrics{};
    const RmdStatus status =
        fixture.valid && compact_oracle_output(fixture, expected)
            ? execute_rmd_stripe_reference(fixture.args, *fixture.packet,
                                           actual, &metrics)
            : RmdStatus::invalid_packet;
        bool case_ok = gather_ok;
    if (test.bits == 16 && test.family == WeightFamily::HP1) {
      ok = check(status == RmdStatus::unsupported_route &&
                     actual.j_padded == 7 &&
                     actual.values == std::vector<OutputValue>{91, 92, 93} &&
                     metrics.packet_call_count == 0,
                 "A16 HP1 compact execution is explicitly outside the Gemmini "
                 "SCU contract") &&
           case_ok && ok;
      continue;
    }
        case_ok = check(status == RmdStatus::success,
                    "matched-width compact oracle executes") &&
              case_ok;
    case_ok =
        check(actual.domain == CompressedOutput::Domain::block_scaled_int64 &&
                            actual.values == expected,
              "compact output matches independent block/lane oracle") &&
        case_ok;
    case_ok =
        check(
            metrics.packet_call_count == 1 && metrics.ws_call_count == 0,
            "software compact executes one packet and no hardware dispatch") &&
        case_ok;
        if (status == RmdStatus::success) {
      ...
      case_ok =
          check(compose_rmd_output(*fixture.packet, actual, composed) ==
                                RmdStatus::success &&
                    execute_rmd_stripe_reference(
                        fixture.args, *fixture.packet, streamed,
                        &streaming_metrics) == RmdStatus::success &&
                    direct_outputs_match(streamed, composed),
                "streaming executor equals compact execution plus compose") &&
          case_ok;
      case_ok = check(streaming_metrics.packet_call_count ==
                              metrics.packet_call_count &&
                          streaming_metrics.matmul_call_count ==
                              metrics.matmul_call_count &&
                          streaming_metrics.physical_tile_count ==
                              metrics.physical_tile_count &&
                          streaming_metrics.lane_group_count ==
                              metrics.lane_group_count,
                      "streaming preserves execution geometry metrics") &&
                case_ok;
      streamed = PreScaledFloat64Correction{{-29.5}};
      case_ok =
          check(stream_compressed_reverse(*fixture.packet, actual, streamed) ==
                                RmdStatus::success &&
                                direct_outputs_match(streamed, composed),
                "reverse physical tile order preserves all logical rows and "
                "columns") &&
                case_ok;
        }
    ...
    case_ok =
        check(packet_block_count == 2 &&
              ...
              "compact packet preserves K0>1 and final partial K fragment") &&
        case_ok;
        const bool partition_expected = kArrayDim < kBlockSize;
    case_ok = check(metrics.active_lanes == 2 * fixture.active_lane_count &&
                            metrics.lane_group_count >= packet_block_count &&
                            (!partition_expected ||
                             metrics.lane_group_count > packet_block_count) &&
                            metrics.matmul_call_count >= metrics.lane_group_count,
                    "compact packet exercises DIM-aware lane groups") &&
              case_ok;

        const std::array<size_t, 7> geometry = {
        metrics.packet_call_count, metrics.active_blocks,
        metrics.active_lanes,      metrics.compact_k_count,
        metrics.padded_k_count,    metrics.matmul_call_count,
            metrics.lane_group_count,
        };
      ...
      case_ok =
          check(geometry == invariant_geometry[geometry_index],
                            "compact packet counts are independent of weight family") &&
                case_ok;
        }
    std::printf("COMPACT_ORACLE width=%u family=%s domain=block_scaled_int64 "
            "packets=%zu k_fragments=%zu lane_groups=%zu\n",
                static_cast<unsigned>(test.bits),
                weight_family_name(test.family), metrics.packet_call_count,
                metrics.matmul_call_count, metrics.lane_group_count);
        ok = case_ok && ok;
    }
    return ok;
...
bool test_full_int32_compact_direct_agreement(size_t rows) {
    ...
    for (uint8_t bits : {uint8_t{4}, uint8_t{8}, uint8_t{16}}) {
        for (WeightFamily family : {WeightFamily::H1, WeightFamily::HP1}) {
          ...
          std::numeric_limits<int32_t>::min(),
          std::numeric_limits<int32_t>::max(),
          static_cast<int32_t>(carry_boundary),
          static_cast<int32_t>(carry_boundary + 1),
          -(int32_t{1} << 20) - 1,
          int32_t{1} << 20,
          0,
          1,
            ...
            if (!check(fixture.args.A.allocate(values.size(), kBlockSize, bits),
                 "full int32 fixture activation storage allocates"))
        return false;
      fixture.args.act_quant.storage().emplace<act::tensor::Meta>().scale =
          0.5f;
          ...
          return check(
              false,
              "full int32 compact and direct builders accept all events");
                }
            }
            const auto packet = compact_builder.finish();
            const auto direct_payload = direct_builder.finish();
            if (!check(packet != nullptr && direct_payload != nullptr,
                 "full int32 compact and direct payloads build"))
        return false;
            ...
            CompressedOutput compressed;
            RmdExecutionMetrics metrics{};
      if (bits == 16 && family == WeightFamily::HP1) {
        ok = check(execute_rmd_stripe_reference(fixture.args, *packet, streamed,
                                                &metrics) ==
                           RmdStatus::unsupported_route &&
                       std::get<PreScaledFloat64Correction>(streamed).values ==
                           std::vector<double>{-7.5} &&
                       residual::execute_direct_stripe(
                           fixture.args, *direct_payload, direct) ==
                           RmdStatus::success,
                   "A16 NPU is unsupported while explicit full-int32 "
                   "CPU-direct remains available") &&
             ok;
        continue;
      }
      if (!check(
              execute_rmd_stripe_reference(fixture.args, *packet, streamed,
                                           &metrics) == RmdStatus::success &&
                  execute_rmd_stripe_reference(fixture.args, *packet,
                                               compressed) ==
                      RmdStatus::success &&
                  compose_rmd_output(*packet, compressed, composed) ==
                      RmdStatus::success &&
                  residual::execute_direct_stripe(fixture.args, *direct_payload,
                                                  direct) == RmdStatus::success,
              "full int32 executes through streaming, compose and CPU direct"))
        return false;
      ...
      ok = check(std::get<BlockScaledInt64Correction>(streamed).values ==
                         expected &&
                           direct_outputs_match(streamed, direct) &&
                           direct_outputs_match(streamed, composed),
                 "top carry correction matches literal int64 and independent "
                 "direct output") &&
           ok;
      ...
      const size_t group_rows =
                    ...
                    for (size_t col = 0; col < packet->j_padded; ++col) {
                        if (row >= rows || col >= packet->logical_j) {
                            const size_t index = block.output_value_offset +
                                   ...
                                   row * packet->j_padded + col;
                            ok = check(compressed.values[index] == 0,
                         "dense input groups preserve zero compressed output "
                         "padding") &&
                   ok;
                        }
                    }
        ...
        const float expected_output =
            7.0f +
            static_cast<float>(static_cast<double>(expected[row]) * 0.25 * 0.5);
                ok = check(output[row] == expected_output,
                   "full int32 merge matches explicit column and activation "
                   "scales") &&
             ok;
            }
        }
...
bool test_shared_weight_merge_preparation() {
    namespace wreader = ggml::gemmini::quants::wreader;
    namespace act = ggml::gemmini::quants::act;
  ...
  auto &args = fixture.args;
...
#if GGML_GEMMINI_WEIGHT_BITS == 4
    std::fill(std::begin(blocks[block].qs), std::end(blocks[block].qs),
              uint8_t{0x88});
    ...
    for (size_t stripe = 0; stripe < packets.size(); ++stripe) {
        ...
        RmdStripeBuilder builder;
    builder.reset(stripe, stripe, 1, args.K, args.J,
                  GGML_GEMMINI_ACTIVATION_BITS);
    if (!builder.add_residual(0, stripe == 0 ? 0 : kBlockSize,
                              residuals[stripe]))
      return false;
    ...
    if (!check(rmd::detail::execute_rmd_stripe_ws_with_weights(
                     ...
                     rejected_metrics.packet_call_count == 73,
                 "FPGA UART rejects shared WS execution without CPU fallback")) return false;
#endif
    // Isolate cached final-merge preparation from backend dispatch. Numerical
    // execution has independent oracle/provider tests; raw corrections here are
    // literal inputs to the public merge contract, also valid for CPU-direct A16.
    std::array<Correction, 3> corrections;
    for (size_t stripe = 0; stripe < corrections.size(); ++stripe) {
        const int64_t expected = static_cast<int64_t>(residuals[stripe]) * 5 * (stripe == 0 ? 4 : 8);
        corrections[stripe] = BlockScaledInt64Correction{{expected}};
#if GGML_GEMMINI_WEIGHT_BITS != 16
        Correction reference;
        if (!check(execute_rmd_stripe_reference(stripe_args[stripe], *packets[stripe], reference) ==
                       RmdStatus::success && direct_outputs_match(reference, corrections[stripe]),
                   "shared-merge inputs agree with the explicit HP1 numerical reference")) return false;
#endif
    }
  ...
  for (auto &worker : workers)
    worker.join();
  const size_t shared_validations =
      wreader::test_weight_reader_storage_validations();
  bool ok = check(
      shared_validations == 1 && shared.column_preparations() == 1 &&
    ...
    const int64_t expected =
        static_cast<int64_t>(residuals[stripe]) * 5 * (stripe == 0 ? 4 : 8);
        ok = check(statuses[stripe] == RmdStatus::success &&
                   std::get<BlockScaledInt64Correction>(corrections[stripe])
                           .values == std::vector<int64_t>{expected} &&
                   rmd::detail::merge_rmd_correction_with_weights(
                       stripe_args[stripe], fresh_output.data(),
                       *packets[stripe], corrections[stripe],
                       fresh_weights) == RmdStatus::success &&
                   shared_output[stripe] ==
                       7.0f + static_cast<float>(static_cast<double>(expected) *
                                                 0.25 * 0.5 * (stripe + 1)),
               "shared and fresh merge preparations match literal corrections "
               "with current row scales") &&
         ok;
    }
  const size_t fresh_validations =
      wreader::test_weight_reader_storage_validations();
    ok = check(fresh_validations == 3 && shared_output == fresh_output,
             "shared preparation reduces three validations to one with exact "
             "FP32 output") &&
       ok;
    ...
    ok = check(rmd::detail::execute_rmd_stripe_ws_with_weights(
                 stripe_args[0], malformed, correction, shared, &metrics) ==
                     RmdStatus::invalid_packet &&
                 direct_outputs_match(correction, sentinel) &&
                 metrics.packet_call_count == 73,
             "reused preparation retains packet validation and transactional "
             "failures") &&
       ok;
    malformed = *packets[0];
    ++malformed.logical_j;
  ok =
      check(rmd::detail::merge_rmd_correction_with_weights(
                stripe_args[0], shared_output.data(), malformed, corrections[0],
                shared, &count) == RmdStatus::invalid_arguments &&
                   shared_output == output_before && count == 91,
            "shared merge retains per-stripe shape checks without partial "
            "writes") &&
      ok;
    RmdStripeBuilder selected;
    selected.reset(9, 0, 1, args.K, args.J, GGML_GEMMINI_ACTIVATION_BITS);
  if (!selected.add_residual(0, 2 * kBlockSize, 1))
    return false;
    const auto mismatched = selected.finish();
  if (!mismatched)
    return false;
    correction = BlockScaledInt64Correction{{80}}; // residual 1 * weight 5 * 2^4
    ok = check(rmd::detail::merge_rmd_correction_with_weights(
                     stripe_args[0], shared_output.data(), *mismatched,
                     correction, shared,
                     &count) == RmdStatus::unsupported_route &&
                   shared_output == output_before && count == 91 &&
                 rmd::detail::merge_rmd_correction_with_weights(
                     stripe_args[0], shared_output.data(), *packets[0],
                     corrections[0], shared) == RmdStatus::success,
             "only a selected mismatched block rejects, without poisoning "
             "disjoint stripes") &&
       ok;
    std::array<float, 3> public_output{7, 7, 7};
    ok = check(merge_rmd_correction_to(stripe_args[0], public_output.data(),
                                     *packets[0],
                                     corrections[0]) == RmdStatus::success,
             "public packet merge checks column scales only for selected "
             "blocks") &&
       ok;
    const auto public_before = public_output;
  ok = check(merge_rmd_correction_to(stripe_args[0], public_output.data(), 0, 1,
                                     corrections[0]) ==
                     RmdStatus::unsupported_route &&
                 public_output == public_before,
             "public row-range merge checks all block scales without partial "
             "writes") &&
       ok;
  ...
  ok =
      check(rmd::detail::merge_rmd_correction_with_weights(
                    stripe_args[0], fresh_output.data(), *packets[0],
                    corrections[0], next_job) == RmdStatus::success &&
                fresh_output[0] == 168.25f,
            "a fresh job reads changed metadata at the same storage address") &&
      ok;
  // All nonnegative INT16 HP1 exponents are valid SCU carriers. Exercise
  // metadata rejection with an actually invalid negative exponent instead.
  blocks[0].m = -1;
    rmd::detail::RmdWeightPreparation overflow_job;
    correction = sentinel;
    ok = check(rmd::detail::execute_rmd_stripe_ws_with_weights(
                 stripe_args[0], *packets[0], correction, overflow_job,
                 &metrics) == RmdStatus::unsupported_route &&
                 direct_outputs_match(correction, sentinel) &&
                 metrics.packet_call_count == 73,
             "a fresh job rejects invalid HP1 carrier metadata without "
             "changing output metrics") &&
       ok;
  ok = check(execute_rmd_stripe_ws(stripe_args[0], *packets[0], correction) ==
                     RmdStatus::unsupported_route &&
                   direct_outputs_match(correction, sentinel),
             "public execution revalidates changed weight metadata") &&
       ok;
  ok =
      check(merge_rmd_correction_to(stripe_args[0], public_output.data(),
                                    *packets[0], corrections[0]) ==
                    RmdStatus::unsupported_route &&
                   public_output == public_before,
            "public merge revalidates changed scales without partial writes") &&
      ok;
  std::printf("SHARED_PREPARATION stripes=3 shared_validations=%zu "
              "fresh_validations=%zu columns=%zu selected_blocks=%zu\n",
              shared_validations, fresh_validations,
              shared.column_preparations(),
...
bool test_compact_failure_matrix() {
    const CompressedOutput sentinel = {
      CompressedOutput::Domain::block_scaled_int64,
      7,
      {13, -9, 4},
    };
  auto fails_atomically = [&](const ggml_gemmini_args_t &args,
                              const StripePacket &packet, RmdStatus expected,
                              const char *message) {
        CompressedOutput output = sentinel;
        RmdExecutionMetrics metrics{};
    const RmdStatus status =
        execute_rmd_stripe_ws(args, packet, output, &metrics);
    const Correction correction_sentinel =
        PreScaledFloat64Correction{{13.25, -9.5}};
    ...
    return check(
        status == expected && compressed_outputs_match(output, sentinel) &&
    ...
    for (uint8_t bits : {uint8_t{4}, uint8_t{8}, uint8_t{16}}) {
        ...
        ok = fails_atomically(h0.args, *single_packet, RmdStatus::unsupported_route,
                          "non-A8 H0 compact rejects before packet dispatch") &&
         ok;
    }
    ...
    ok = fails_atomically(h2_args, *single_packet, RmdStatus::unsupported_route,
                        "H2 compact rejects before packet dispatch") &&
       ok;
    ...
    ok = fails_atomically(hp2_args, *single_packet, RmdStatus::unsupported_route,
                        "HP2 compact rejects before packet dispatch") &&
       ok;
    ...
    ok =
        fails_atomically(forged_mask.args, malformed, RmdStatus::invalid_packet,
                         "forged K-mask rejects before packet dispatch") &&
        ok;
    }
    ...
    if (mixed.valid) {
    ok = fails_atomically(
             mixed.args, *mixed.packet, RmdStatus::unsupported_route,
             "mixed activation/weight identity rejects before dispatch") &&
         ok;
    }
    ...
    ok = check(mismatched_native.valid,
             "packet/build mismatch compact fixture builds") &&
       ok;
    if (mismatched_native.valid) {
        ...
        ok = check(status == RmdStatus::unsupported_route &&
               ...
               "packet/build width mismatch rejects before Gemmini dispatch") &&
         ok;
    }
  CompactOracleFixture truncated_native(GGML_GEMMINI_WEIGHT_BITS,
                                        WeightFamily::H1);
    ...
    if (truncated_native.valid) {
        ...
        ok = check(rmd::execute_rmd_stripe_gemmini_for_test(
                   truncated_native.args, *truncated_native.packet, output,
                   &metrics) == RmdStatus::unsupported_route &&
                   compressed_outputs_match(output, sentinel) &&
                   metrics.packet_call_count == 0 &&
                   ...
                   ggml::gemmini::quants::wreader::
                           test_weight_reader_code_address_resolutions() == 0,
               "truncated native weights reject before any gather or Gemmini "
               "dispatch") &&
         ok;
    } else {
    ...
    ok = check(wide_source.valid &&
                   gather_weight_tile_for_test(
                       wide_source.args, 1, local_k.data(), local_k.size(), 1,
                       2, tile.data(), kArrayDim,
                       &metrics) == RmdStatus::overflow &&
                       tile == before && metrics.weight_values_gathered == 19 &&
                       metrics.weight_address_resolutions == 23,
               "late code narrowing overflow preserves the whole tile and "
               "metrics") &&
         ok;
    }
#else
  ok = check(false, "compact failure matrix requires GGML_GEMMINI_TESTING") &&
       ok;
#endif
  ...
  ok = check(scale_overflow.valid, "scale overflow compact fixture builds") &&
       ok;
    ...
    ok = check(status == RmdStatus::unsupported_route &&
                   ...
                   metrics.packet_call_count == 0 &&
                   metrics.ws_call_count == 0 && metrics.matmul_call_count == 0,
               "unsupported A16 HP1 execution preserves compressed output and "
               "metrics") &&
         ok;
    const Correction correction_sentinel =
        PreScaledFloat64Correction{{13.25, -9.5}};
        ...
        ok = check(execute_rmd_stripe_reference(scale_overflow.args, *scale_overflow.packet,
                                                correction, &metrics) == RmdStatus::unsupported_route &&
                       direct_outputs_match(correction, correction_sentinel) &&
                       metrics.packet_call_count == 17 && metrics.matmul_call_count == 19 &&
                       metrics.im2p_stats.fields[0] == 23 && metrics.residual_nnz == 29 &&
                       metrics.residual_max == 31 && metrics.digit_nnz == 37 &&
                       metrics.gathered_weight_host_bytes == 41,
                   "unsupported A16 HP1 stream preserves correction and existing metrics") && ok;
#else
    ok = check(false, "scale overflow oracle requires GGML_GEMMINI_TESTING") &&
         ok;
#endif
    }
    ...
    if (malformed.valid) {
        ...
        ok = fails_atomically(malformed.args, packet, RmdStatus::invalid_packet,
                          "malformed compact packet is failure-atomic") &&
         ok;
    }
    ...
    ok = check(gather_wide_weight_tile_for_test(dense, 0, &local_k, 1, 0, 1,
                 ...
                 tile[0] == 71,
             "nonhierarchical gather still validates storage extent before "
             "reading") &&
       ok;
...
static bool test_cpu_capture_is_canonical_and_packet_free() {
  ...
  if (!capture.add_residual(1, 33, -129) || !capture.add_residual(0, 31, 256) ||
    ...
    if (!check(payload.direct != nullptr, "CPU creates a direct payload") ||
      ...
      !check(payload.direct->events.size() == 3,
             "CPU retains every original event")) {
        return false;
    }
...
static bool test_ws_capture_preserves_packet_contract() {
    ...
    return check(payload.packet->k_indices == expected->k_indices,
         ...
         check(payload.packet->stacked_activation ==
                   expected->stacked_activation,
...
static bool test_empty_capture_and_single_sink_selection() {
    ...
    return check(cpu.holds_cpu_sink() && !cpu.holds_ws_sink(),
        ...
        check(ws.holds_ws_sink() && !ws.holds_cpu_sink(),
         ...
         check(cpu_empty.empty() && ws_empty.empty(),
               "empty stripes produce no work") &&
...
static bool test_direct_payload_slicing_and_ownership() {
  ...
  std::vector<DirectStripePayloadHandle> inputs{first.finish().direct,
                                                second.finish().direct};

    RmdStatus status = RmdStatus::success;
  DirectStripePayloadHandle identity =
      slice_direct_payloads(inputs, 10, 12, 4, status);
    if (!check(status == RmdStatus::success && identity == inputs[0],
               "exact direct slice preserves payload identity")) {
        return false;
    }
  DirectStripePayloadHandle slice =
      slice_direct_payloads(inputs, 11, 13, 9, status);
    ...
    return check(slice->events.size() == 2, "slice keeps matching events") &&
         check(slice->events[0] == ResidualEvent{0, 5, 12},
               "slice renormalizes first local row") &&
         check(slice->events[1] == ResidualEvent{1, 6, 13},
               "slice renormalizes second local row") &&
...
static bool test_direct_payload_validation_rejects_malformed_contracts() {
  ...
  bool ok =
      check(validate_direct_payload(malformed) == RmdStatus::invalid_packet,
...
static TestSelection parse_selection(int argc, char **argv) {
  ...
  if (std::strcmp(argv[1], "--case=happy-table") == 0)
    return TestSelection::happy_table;
  if (std::strcmp(argv[1], "--case=failure-table") == 0)
    return TestSelection::failure_table;
  if (std::strcmp(argv[1], "--case=direct-happy") == 0)
    return TestSelection::direct_happy;
  if (std::strcmp(argv[1], "--case=direct-failure") == 0)
    return TestSelection::direct_failure;
  if (std::strcmp(argv[1], "--case=compact-happy") == 0)
    return TestSelection::compact_happy;
  if (std::strcmp(argv[1], "--case=compact-failure") == 0)
    return TestSelection::compact_failure;
  if (std::strcmp(argv[1], "--case=radix-happy") == 0)
    return TestSelection::radix_happy;
  if (std::strcmp(argv[1], "--case=radix-failure") == 0)
    return TestSelection::radix_failure;
    return TestSelection::invalid;
}
...
int main(int argc, char **argv) {
  ...
  if (!check(context != nullptr, "GGML initialization succeeds"))
    return 1;
    ggml_free(context);

    bool ok = true;
  if (selection == TestSelection::all ||
  ...
  if (selection == TestSelection::all ||
      selection == TestSelection::compact_happy) {
        ...
        ok = test_shared_weight_merge_preparation() && ok;
    }
  if (selection == TestSelection::all ||
    ...
    const char *message =
        ...
        : selection == TestSelection::radix_failure
            ? "PASS: width-native radix rejection and malformed metadata "
              "atomicity"
        : selection == TestSelection::failure_table
            ? "PASS: RMD residual weight failure table"
        : selection == TestSelection::happy_table
            ? "PASS: RMD residual weight happy table"
            : "PASS: RMD residual weight plus matched-width direct and compact "
              "matrices";
        std::puts(message);
    }
    return ok ? 0 : 1;
