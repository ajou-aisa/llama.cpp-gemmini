...
wroute::WeightScaleInfo
build_weight_scale_info_impl(const ggml_gemmini_args_t &args,
  ...
  if (mode == wroute::WeightScaleInfoMode::CommonOutput &&
      (args.weight_format == Format::q8_h2 ||
       args.weight_format == Format::q8_hp2)) {
    const size_t rows = args.J;
    const size_t cols = args.weight_format == Format::q8_h2
                            ? args.q8_h2_blocks_per_row
                            : args.q8_hp2_blocks_per_row;
      ...
      for (size_t block_index = 0; block_index < cols; ++block_index) {
        if (args.weight_format == Format::q8_h2) {
          ...
          weight_scales[row * cols + block_index] =
              block->m == INT16_MIN
                  ? 0.0f
                  : gemmini_ldexp_fast_pos(block->channel_scale,
                                           static_cast<int>(block->m));
        }
      }
    }
  ...
  result.rows = mode != wroute::WeightScaleInfoMode::CommonOutput
                    ? args.blocks_J
                    : (args.blocks_J ? args.blocks_J : args.J);
  result.cols = args.blocks_K;
  result.block_size = args.block_size_k ? args.block_size_k : QK8_0;
  return result;
}

bool output_offset(size_t row, size_t col, size_t row_stride, size_t col_stride,
                   size_t &offset) {
  size_t row_offset = 0;
  size_t col_offset = 0;
...
bool checked_k_end(size_t k_offset, size_t block_k, size_t &k_end) {
  if (block_k == 0 ||
      k_offset > std::numeric_limits<size_t>::max() - (block_k - 1))
    return false;
...
bool has_complete_scale_metadata(const wroute::WeightScaleInfo &scales,
...
wroute::WeightRouteKind classify_route(const ggml_gemmini_args_t &args,
                                       wroute::WeightScaleInfoMode mode,
  ...
  auto accept_family = [&](Route route, uint8_t bits, bool native_blocks) {
    const bool integer_domain =
        route == Route::H1 ||
        (route == Route::HP1 &&
         mode != wroute::WeightScaleInfoMode::CommonOutput);
  ...
  case Format::q4_h0:
    return args.has_native_matched_width_contract()
               ? accept_family(Route::H0, 4, true)
               : invalid("invalid q4_h0 contract");
  case Format::q4_h1:
    return args.has_native_matched_width_contract()
               ? accept_family(Route::H1, 4, true)
               : invalid("invalid q4_h1 contract");
  case Format::q4_hp1:
    return args.has_native_matched_width_contract()
               ? accept_family(Route::HP1, 4, true)
               : invalid("invalid q4_hp1 contract");
  case Format::q8_h0:
    return accept_family(Route::H0, 8, args.B_blocks != nullptr);
  case Format::q8_h1:
    return args.has_q8_h1_im2p_contract() ? accept_family(Route::H1, 8, true)
                                          : invalid("invalid q8_h1 contract");
  case Format::q8_hp1:
    return wroute::has_q8_hp1_native_contract(args)
               ? accept_family(Route::HP1, 8, true)
               : invalid("invalid q8_hp1 contract");
  case Format::q16_h0:
    return args.has_native_matched_width_contract()
               ? accept_family(Route::H0, 16, true)
               : invalid("invalid q16_h0 contract");
  case Format::q16_h1:
    return args.has_native_matched_width_contract()
               ? accept_family(Route::H1, 16, true)
               : invalid("invalid q16_h1 contract");
  case Format::q16_hp1:
    return args.has_native_matched_width_contract()
               ? accept_family(Route::HP1, 16, true)
               : invalid("invalid q16_hp1 contract");
  case Format::q8_h2:
    if (mode != wroute::WeightScaleInfoMode::CommonOutput) {
...
wroute::WeightRouteStatus
route_status_from_reader(wreader::WeightReaderStatus status) {
  switch (status) {
...
WeightRoutePlan resolve_weight_route_plan(const ggml_gemmini_args_t &args,
                                          WeightScaleInfoMode mode) {
  WeightRoutePlan plan{};
  plan.route = classify_route(args, mode, plan);
  plan.hp1_carriers = mode == WeightScaleInfoMode::ResidualHp1Scu &&
                      plan.route == WeightRouteKind::HP1;
  if (plan.route == WeightRouteKind::Unsupported)
    return plan;

  plan.layout = plan.route == WeightRouteKind::Dense
                    ? (args.transpose_B ? WeightLayout::JxK_ColMajor
                                        : WeightLayout::KxJ_RowMajor)
                    : WeightLayout::JxK_ColMajor;
  plan.weight_stride =
      plan.native_weight_blocks || plan.route == WeightRouteKind::H0 ||
              plan.route == WeightRouteKind::H1 ||
              plan.route == WeightRouteKind::HP1
          ? args.K
          : (args.sB ? args.sB
                     : (plan.layout == WeightLayout::JxK_ColMajor ? args.K
                                                                  : args.J));
  ...
  if ((plan.route == WeightRouteKind::H0 || plan.route == WeightRouteKind::H1 ||
       plan.route == WeightRouteKind::HP1) &&
      !block_scales) {
    plan.status = WeightRouteStatus::InvalidMetadata;
  ...
  if (mode != WeightScaleInfoMode::CommonOutput && block_scales) {
    const size_t required_scale_cols =
        args.K == 0 ? 0 : 1 + (args.K - 1) / plan.scales.block_size;
  ...
  if (plan.route == WeightRouteKind::H0 || plan.route == WeightRouteKind::H1 ||
      plan.route == WeightRouteKind::HP1) {
    const wreader::WeightReaderStatus reader_status =
    ...
    plan.scale_domain = block_scales
                            ? WeightScaleDomain::FloatingBlock
                            : WeightScaleDomain::IntegerBlockTimesColumn;
    plan.cpu_direct_capable = true;
    plan.compact_capable =
...
WeightRouteStatus weight_route_status(const WeightRoutePlan &plan,
                                      WeightExecutionPath path) {
  if (!plan.valid)
    return plan.status;
  const bool capable = path == WeightExecutionPath::CpuDirect
                           ? plan.cpu_direct_capable
                           : plan.compact_capable;
  return capable ? WeightRouteStatus::Success
                 : WeightRouteStatus::UnsupportedExecution;
}

const char *weight_route_status_name(WeightRouteStatus status) {
  switch (status) {
  case WeightRouteStatus::Success:
    return "success";
  case WeightRouteStatus::UnsupportedFormat:
    return "unsupported-format";
  case WeightRouteStatus::InvalidMetadata:
    return "invalid-metadata";
  case WeightRouteStatus::UnsupportedExecution:
...
const char *weight_route_kind_name(const WeightRoutePlan &plan) {
  ...
  case WeightRouteKind::Q8ChannelDirect:
    return "q8-channel-direct";
  case WeightRouteKind::Q8ChannelSidecar:
    return "q8-channel-sidecar";
  case WeightRouteKind::H0:
    return "h0";
  case WeightRouteKind::H1:
    return "h1";
  case WeightRouteKind::HP1:
    return "hp1";
  case WeightRouteKind::Unsupported:
    return "unsupported";
  }
  return "unsupported";
}
...
bool route_covers_k(const WeightRoutePlan &plan, size_t k_count) {
  if (!plan.valid || plan.scales.scalar_mode || plan.scales.row_header_mode ||
      plan.scales.channel_mode)
    return plan.valid;
  return k_count == 0 ||
...
bool route_block_for_range(const WeightRoutePlan &plan, size_t k_offset,
                           size_t block_k, size_t &block_index) {
  block_index = 0;
  if (!plan.valid || block_k == 0)
    return false;
  if (plan.scales.scalar_mode || plan.scales.row_header_mode ||
      plan.scales.channel_mode)
    return true;
...
float route_weight_scale(const WeightRoutePlan &plan,
                         const ggml_gemmini_args_t &args, size_t j,
                         size_t block_index) {
  if (plan.scales.row_header_mode)
  ...
  if (plan.route == WeightRouteKind::H0 || plan.route == WeightRouteKind::H1 ||
    ...
    return static_cast<float>(static_cast<double>(scale.integer_block_scale) *
...
uint64_t route_block_scale(const WeightRoutePlan &plan,
                           const ggml_gemmini_args_t &args, size_t j,
                           size_t block_index) {
...
float route_column_scale(const WeightRoutePlan &plan,
                         const ggml_gemmini_args_t &args, size_t j) {
  if (plan.scales.row_header_mode)
    return args.q8_channel_scale(j);
  if (plan.scales.scalar_mode)
  ...
  if (plan.hp1_carriers) {
    const auto metadata = wreader::read_hp1_carrier_validated(args, plan, j, 0);
    return metadata.ok() ? metadata.column_scale
...
void dequantize(const ggml_gemmini_args_t &args, size_t k_offset,
                size_t block_k, const int32_t *acc32, size_t acc_stride) {
  if (!args.f_out || !acc32 || args.I == 0 || args.J == 0 || block_k == 0 ||
      acc_stride == 0)
    return;
  ...
  if (!quants::wroute::route_block_for_range(plan, k_offset, block_k,
                                             first_block))
    return;
