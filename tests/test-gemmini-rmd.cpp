  ...
  for (uint8_t bits : {uint8_t{4}, uint8_t{8}, uint8_t{16}}) {
    for (WeightFamily family : {WeightFamily::H1, WeightFamily::HP1}) {
      ...
      ok = check(merge_rmd_correction(fixture.args, *packet, streamed,
                                      &nonzero_count) == RmdStatus::success &&
                     nonzero_count == values.size(),
                 "full int32 correction merges with final nonzero count") &&
           ok;
      for (size_t row = 0; row < values.size(); ++row) {
...
bool test_shared_weight_preparation() {
    ...
    workers[stripe] = std::thread([&, stripe] {
      ...
      if (statuses[stripe] == RmdStatus::success) {
        statuses[stripe] = rmd::detail::merge_rmd_correction_with_weights(
            stripe_args[stripe], shared_output.data(), *packets[stripe],
            corrections[stripe], shared);
      }
    });
  }
  ...
  for (size_t stripe = 0; stripe < packets.size(); ++stripe) {
    ...
    ok = check(statuses[stripe] == RmdStatus::success &&
                   ...
                   rmd::detail::execute_rmd_stripe_ws_with_weights(
                       stripe_args[stripe], *packets[stripe], fresh,
                   ...
                   rmd::detail::merge_rmd_correction_with_weights(
                       ...
                       *packets[stripe], fresh,
                       fresh_weights) == RmdStatus::success &&
                   shared_output[stripe] ==
                       7.0f + static_cast<float>(static_cast<double>(expected) *
                                                 0.25 * 0.5 * (stripe + 1)),
               "shared stripes match fresh execution and literal correction "
  ...
  if (!mismatched)
    return false;
  ok = check(rmd::detail::execute_rmd_stripe_ws_with_weights(
                 stripe_args[0], *mismatched, correction, shared) ==
                     RmdStatus::success &&
                 rmd::detail::merge_rmd_correction_with_weights(
  ...
  ok =
      check(rmd::detail::execute_rmd_stripe_ws_with_weights(
                stripe_args[0], *packets[0], correction, next_job) ==
                    RmdStatus::success &&
                rmd::detail::merge_rmd_correction_with_weights(
                    stripe_args[0], fresh_output.data(), *packets[0],
                    correction, next_job) == RmdStatus::success &&
...
bool test_compact_failure_matrix() {
  ...
  if (scale_overflow.valid) {
    ...
    ok = check(execute_rmd_stripe_reference(
                   scale_overflow.args, *scale_overflow.packet, correction,
                   &metrics) == RmdStatus::unsupported_route &&
                   direct_outputs_match(correction, correction_sentinel) &&
                   metrics.packet_call_count == 17 &&
                   metrics.matmul_call_count == 19 &&
                   metrics.im2p_stats.fields[0] == 23,
               "unsupported A16 HP1 stream preserves correction and existing "
               "metrics") &&
         ok;
#else
...
static bool test_cpu_capture_is_canonical_and_packet_free() {
  ...
  return check(events[0] == ResidualEvent{0, 2, 1},
               "CPU event order row 0 k 2") &&
         check(events[1] == ResidualEvent{0, 31, 256},
               "CPU event order row 0 k 31") &&
         check(events[2] == ResidualEvent{1, 33, -129},
               "CPU event order row 1 k 33");
}
...
int main(int argc, char **argv) {
  ...
  if (selection == TestSelection::all ||
    ...
    for (size_t rows : {size_t{1}, size_t{2}, size_t{8}, kArrayDim - 1,
                        kArrayDim, kArrayDim + 1}) {
