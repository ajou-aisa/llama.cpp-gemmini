...
if(NOT direct_run_count EQUAL 3)
    message(FATAL_ERROR "Two event bindings and one FullCPU metadata binding are required")
endif()
set(event_bindings 0)
set(metadata_bindings 0)
foreach(binding IN LISTS direct_run_bindings)
    if(binding STREQUAL "direct_metrics.run_id = event.run_id")
        math(EXPR event_bindings "${event_bindings} + 1")
    elseif(binding STREQUAL "direct_metrics.run_id = metadata->run_id")
        math(EXPR metadata_bindings "${metadata_bindings} + 1")
    else()
        message(FATAL_ERROR "Direct worker identity must come from its event or FullCPU metadata")
    endif()
endforeach()
if(NOT event_bindings EQUAL 2 OR NOT metadata_bindings EQUAL 1)
    message(FATAL_ERROR "FULL/PIPELINE event identities and FullCPU metadata identity must all be preserved")
endif()
string(FIND "${im2p_source}" "\napply_baseline_rmd_full(" baseline_full_start)
if(baseline_full_start EQUAL -1)
    message(FATAL_ERROR "FullCPU residual function missing")
endif()
string(SUBSTRING "${im2p_source}" ${baseline_full_start} -1 baseline_full_tail)
string(FIND "${baseline_full_tail}" "\n}\n" baseline_full_end)
if(baseline_full_end LESS 0)
    message(FATAL_ERROR "FullCPU residual function boundary missing")
endif()
string(SUBSTRING "${baseline_full_tail}" 0 ${baseline_full_end} baseline_full_body)
string(REGEX MATCH "if [(]metadata != nullptr[)][ \n\r]*direct_metrics[.]run_id = metadata->run_id"
       baseline_full_metadata "${baseline_full_body}")
if(NOT baseline_full_metadata)
    message(FATAL_ERROR "FullCPU identity must use its owned metadata while preserving run_id zero")
endif()
