...
namespace
{
    ...
    bool set_scale(size_t stripe, const StripeStats &stats, Meta &meta)
    {
        ...
        const float scale = static_cast<float>(stats.max_abs / static_cast<float>(ggml::gemmini::config::GGML_GEMMINI_ACTIVATION_QMAX));
...
bool dequantize_activation(
    ...
    const ggml_gemmini_args_t &args)
{
    if (!args.A.valid() || !dst || args.I == 0 || args.K == 0 || dst_row_stride == 0 || dst_col_stride == 0 || rows == 0 || cols == 0)
        return false;
