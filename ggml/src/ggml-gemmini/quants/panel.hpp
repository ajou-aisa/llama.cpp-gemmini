...
//
// Panel struct for GEMM tiling geometry - Pure algorithm layer (ggml-free)
...
struct Panel
{
    ...
    // Helper: ceiling division for panel count
    static size_t ceil_div(size_t total, size_t tile_size)
    {
        return (total + tile_size - 1) / tile_size;
    }

    // Number of panels needed to cover extent
    static size_t panels_count(size_t total, size_t tile_size)
    {
        return ceil_div(total, tile_size);
    }

    // Create panel from tile index (row-major ordering)
    //
    // tile_idx: zero-based tile index
    // extent_i, extent_j: total tensor dimensions
    // tile_i, tile_j: hardware tile dimensions
    //
    // Returns panel with clamped extent for edge tiles.
    static Panel from_index(
        size_t tile_idx,
        size_t extent_i,
        size_t extent_j,
        size_t tile_i,
        size_t tile_j)
    {
        const size_t num_tiles_j = panels_count(extent_j, tile_j);
        ...
        return Panel(i, j, row_off, col_off);
    }
};
} // namespace ggml::gemmini::quants
